...
np.set_printoptions(precision=3) #set precsion

channel = EngineConfigurationChannel()
env = UnityEnvironment(file_name = './env/football', side_channels=[channel])
channel.set_configuration_parameters(time_scale = 4)  #speed
env.reset()

load_tensor = True

two_striker = True
num_episodes = 150   #games
num_game_each_episode = 1000  #flims
epsilon = 1.0                 #randomness
epsilon_min = 0.05
epsilon_decay = 0.99
scores = [[],[]]              #multi dimensional arrays
best_avg_score = 0
...
class Agent():
    def __init__(self, state_size, action_size, behavior_name, index_player, replay_memory_size=1e4, batch_size=128, gamma=0.99,
        ...
        self.seed = random.seed(seed)    #rand
        self.behavior_name = behavior_name
        self.index_player = index_player
        self.close_ball_reward = 0       #rewards
        self.touch_ball_reward = 0       #rewards
    ...
    def Read(self):
        ...
        try:
            ...
            self.close_ball_reward = 0.1 if count_close_to_ball > 0 else 0  ##############
...
def process_reward_list_striker(reward_list): #reward
    # Reward for Scoring
    if reward_list[0] > 0:
        return 10
...
try:
    for i_episode in range(1, num_episodes + 1):
        ...
        list_reward_b1 = []
        list_reward_b2 = []
        for i_game in range(num_game_each_episode):   #do
            ...
            next_state_p1 = agent_p1.Read()  #read
